...
function EditorComponent(props) {
  ...
  const getCurrentNote = (note_id: any) => {
    console.log("Getting note", note_id);
    setIsLoadingUpdate(true);
    fetch(`${props.backendURL}/api/notes/${note_id}`)
    .then(res => res.json())
    .then(res => {
        let currentNoteText: any = props.note.body
        if(res.response.body != props.note.body){
            props.setUpdatedNote(res.response);
            currentNoteText = res.response.body;
        }
        setIsLoadingUpdate(false);
        setEditorState(EditorState.createWithContent(ContentState.createFromText(currentNoteText)));
    })
    .catch((err) => {
        if(err){
            setIsLoadingUpdate(false);
            setConnectionMessage("Cannot able to get updated, notes - Please try later !")
  ...
  useEffect(() => {
    if (!(props.note && props.note.note_id)) return;

    getCurrentNote(props.note.note_id)
    ...
    socketCon.on("connect", () => {
      ...
      setIsConnected(true);
      setConnectionMessage(
        `Connected to note with connection id : ${socketCon.id}`
      );
    ...
    socketCon.on("client::update", (data: any) => {
      ...
      setEditorState(
        EditorState.createWithContent(
          ContentState.createFromText(data.response.body)
        )
      );
      setIsLoadingUpdate(false);
    ...
    socketCon.on("connect_error", () => {
      ...
      setConnectionMessage(
        "Error occrued while conntecting ! Please try later"
      );
  ...
  useEffect(() => {
    ...
    setEditorState(
      EditorState.createWithContent(
        ContentState.createFromText(currentNote.body)
      )
    );
  }, []);
  ...
  return (
    <Container>
      ...
      <Button disabled={!isConnected || isLoadingUpdate} onClick={updateNote}>
        {isLoadingUpdate ? <CircularProgress /> : `Submit`}
      </Button>
    </Container>
  );
}
